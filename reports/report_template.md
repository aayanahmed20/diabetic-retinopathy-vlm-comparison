...
> `notebooks/DR_VLM_Comparison.ipynb` against `results/metrics_summary.json` and
> `results/predictions.csv`. Do not estimate or approximate these numbers — an IEEE
> reviewer (or anyone re-running your code) will notice if reported numbers don't match
> the artifact.
...
Diabetic retinopathy (DR) is retinal vascular damage caused by prolonged hyperglycemia
and is a leading cause of vision loss among working-age adults worldwide, affecting an
...
| RetiZero | Local inference | Zero-shot image–text similarity against the 5 ICDR grade-name labels |

All three models received the same images. Gemini and MedGemma received an identical
prompt encoding the ICDR rubric (Appendix); RetiZero is architecturally a
retrieval/similarity model rather than a generative one, so its predictions are not
directly comparable in mechanism even when scored on the same metric — see Limitations.

### D. Metrics

- **Accuracy** — exact grade match
...
| Model | N scored | Accuracy | Quadratic-weighted κ | MAE (grades) |
|---|---|---|---|---|
...
systematically miss grade 4 / over-call grade 0?]*
...
- **Sample size.** *[FILL IN actual N]* images is [adequate/inadequate — state which,
  with justification] for statistically robust per-class estimates; report confidence
  intervals if N < ~100.
...
[1] Various, "Diabetic retinopathy overview," synthesized from Mayo Clinic and NIH
    sources — replace with formal peer-reviewed citations before submission.
[2] Mayo Clinic, "Diabetic retinopathy — Symptoms & causes."
[3] M. D. Abràmoff et al., grader-variability discussion in DR ML evaluation literature.
[4] Google, "MedGemma model card," Health AI Developer Foundations, 2025.
[5] M. Wang et al., "Enhancing diagnostic accuracy in rare and common fundus diseases
...
*[Replace [1]-[3] with proper formal citations — pull DOIs/full bibliographic entries
before submission; these are placeholders pointing at the source material used to draft
